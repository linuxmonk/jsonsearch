...
// This version only compares the key to string
// keys. The other allowed key value types have
...
func (jdb *JsonDB) rquery(dbname, key string) (*IndexBackend, error) {
		...
		for _, lobj := range jsonType.list {
			...
			if found {
				...
				case int:
					...
					result.stringIndex[sval] = lobj
				case float64:
					sval := strconv.Itoa(int(indexVal))
					result.stringIndex[sval] = lobj
				case string:
					result.stringIndex[indexVal] = lobj
...
// is not known
func (jdb *JsonDB) query(dbname, key string) ([]map[string]interface{}, error) {
